...
func NewAppLookup(ns string, lister corelister.EndpointsLister) *AppLookup {
	return &AppLookup{
		DefaultNamespace: ns,
...
func (f *AppLookup) GetLister(ns string) corelister.EndpointsNamespaceLister {
	f.lock.RLock()
	defer f.lock.RUnlock()
	return f.Listers[ns]
}

func (f *AppLookup) SetLister(ns string, lister corelister.EndpointsNamespaceLister) {
	f.lock.Lock()
	defer f.lock.Unlock()
	f.Listers[ns] = lister
}
...
func (l *AppLookup) Resolve(name string) (url.URL, error) {
	functionName := name
...
func (l *AppLookup) verifyNamespace(name string) error {
	if name != "kube-system" {
		return nil
	}
